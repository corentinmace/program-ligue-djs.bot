...
const servToken = config.servToken
const channelmod = config.channelmod
const logs = config.logs
const roch = config.rolechan

const rolejs = require('./roles.js')
const testRole = rolejs.testRole
...
const version = 'v2.2.0' // Creation d'une variable de la version du bot
...
bot.on('ready', () => {
  bot.user.setActivity(`${prefix}help - ${bot.guilds.get(servToken).memberCount} programmeurs !`, { type: 'WATCHING' })
  ...
  // Roles
  const RChan = bot.guilds.get(servToken).channels.get(roch)
...
bot.on('message', message => {
  ...
  if (message.content === prefix + 'langage') { // Envoi un mp a l'utilisateur avec les langages du serveur sous forme d'embed
    message.delete()
